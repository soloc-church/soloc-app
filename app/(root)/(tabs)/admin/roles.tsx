...
type RoleType = Database["public"]["Enums"]["contextual_role_type"];
...
export default function RoleManagement() {
  ...
  const tabs = useMemo(
    () => [
      { key: "ministry" as const, label: "Ministries", icon: icons.home },
      { key: "team" as const, label: "Teams", icon: icons.person },
      { key: "group_chat" as const, label: "Groups", icon: icons.chat },
    ],
    []
  );
  ...
  const handleAssignRole = async () => {
    ...
    try {
      ...
      if ('error' in res) {
        throw new Error(res.error ?? "RPC failed");
      }
              ...
              {tabs.map((tab) => (
                ...
                >
                  <Image
                    ...
                    style={{
                      tintColor: activeTab === tab.key ? "white" : "#6B7280",
                    }}
                  />
                  <Text
                  ...
                  return (
                      ...
                      onPress={() => {
                        if (!canAssignHere) {
                          Alert.alert(
                            ...
                            "You can't assign roles in this scope."
                          );
                          return;
                        }
                    ...
                    >
                      <View className="flex-row items-center justify-between">
                        ...
                        <View
                          className={`${
                            canAssignHere ? "bg-primary-100" : "bg-gray-100"
                ...
                <View className="flex-row gap-2">
                  ...
                  <TouchableOpacity
                    ...
                    className={`flex-1 rounded-xl py-3 ${
                      selectedScope &&
                      can.assignInScope(authz, selectedScope.type, selectedScope.id)
                        ? "bg-primary-500"
                        : "bg-gray-300"
                    }`}
                    disabled={
                      ...
                      !can.assignInScope(
                        authz,
                        selectedScope?.type as ScopeType,
                        selectedScope?.id as string
                      )
                    }
                  >
                    {assigning ? (
