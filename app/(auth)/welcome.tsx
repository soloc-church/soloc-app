    ...
    return (
        <SafeAreaView className="flex h-full items-center justify-between bg-white">
            ...
            <Swiper
                ref={swiperRef}
                loop={false}
                dot={<View className="w-[32px] h-[4px] mx-1 bg-[#E6E6E6]" />}
                activeDot={<View className="w-[32px] h-[4px] mx-1 bg-[#A89BB5]"/>}
                onIndexChanged={(index)=> setActiveIndex(index)}
            >
                {onboarding.map((item) => (
                    <View key={item.id} className="flex items-center justify-center -5">
                        <Image
                            source={item.image}
